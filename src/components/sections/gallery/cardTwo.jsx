...
const cardTwoItems = [
    { src: portfolio_1, title: "Vocabulary Building Session", col: "sm:col-start-1 sm:col-end-8 col-span-full" },
    { src: portfolio_2, title: "Reading Passage Practice", col: "sm:col-start-8 sm:col-end-13 col-span-full" },
    { src: portfolio_3, title: "Writing Skills Workshop", col: "sm:col-start-1 sm:col-end-7 col-span-full" },
    { src: portfolio_5, title: "Speaking & Expression", col: "sm:col-start-7 sm:col-end-13 col-span-full" },
]
...
const CardTwo = ({ data_target }) => {
    return (
        <div className={`grid lg:gap-7.5 gap-4 grid-cols-12 sm:grid-rows-[453px] top-0 left-0 transition-all duration-500 ${data_target === "school" ? "relative translate-y-0 visible opacity-100" : "absolute translate-y-10 invisible opacity-0"}`}>
            {cardTwoItems.map(({ src, title, col }) => (
